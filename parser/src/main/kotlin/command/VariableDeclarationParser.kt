...
class VariableDeclarationParser : Parser {
    ...
    private fun parseIdentifier(token: Token): IdentifierNode {
        return IdentifierNode(token.value.toString(), token.line, token.column)
    }

    // Handles complex cases with multiple arguments
    private fun handleComplexArgs(identifierNode: IdentifierNode, args: List<Token>, tokens: List<Token>):
    ...
    private fun wrapInParentheses(args: List<Token>): List<Token> {
        return listOf(Token(TokenType.LEFT_PARENTHESIS, TokenValue.StringValue("("), 0, 0)) +
            args +
    ...
    private fun createVariableDeclarationNode(
        ...
        return VariableDeclarationNode(identifierNode, tokenType, expressionNode, token.line, token.column)
    }
